...
async function sendTransactionViaExtension(
  ...
  try {
    ...
    if (receipt) {
      return { status: TransactionState.Sent, receipt };
    } else {
      return { status: TransactionState.Failed, receipt: null };
    }
  } catch (e) {
    console.log(e);
    return { status: TransactionState.Rejected, receipt: null };
  }
}
...
async function sendTransactionViaWallet(
  ...
  if (receipt) {
    ...
    return { status: TransactionState.Failed, receipt };
  }
}
